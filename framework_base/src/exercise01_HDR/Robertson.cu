...
__global__ void countValuesKernel(uint8_t* values, bool* underexposed_mask, uint32_t* counters, uint32_t number_values, uint32_t values_per_image)
{
    ...
    atomicAdd(counters + values[gid], 1);  // for each value the number of pixels of that value is stored here formula 11 in the paper
}
...
__global__ void calcWeightsKernel(uint8_t* values, float* weights, uint32_t number_values)
{
    ...
    float w = fmaxf(expf(-4.0f * nom * nom / denom) - expf(-4.0f), 0.0f); // here the weights from a are calculated see formula 5 in the paper
    weights[gid] = w;
}
...
__global__ void initInvCrfKernel(float* I, uint32_t number_values)
{
    ...
    I[gid] = float(gid); // this linear curve is intialized here from a) 
}
